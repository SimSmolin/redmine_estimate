...
class EstimateEntryQuery < Query
  ...
  self.available_columns = [
    ...
    QueryColumn.new(:tweek, :sortable => ["#{EstimateEntry.table_name}.spent_on", "#{EstimateEntry.table_name}.created_on"], :caption => :label_week),
    ...
    QueryColumn.new(:hours, :sortable => "#{EstimateEntry.table_name}.hours", :totalable => true),
  ]
...
#      left_join_issue.
  ...
  def sql_for_issue_id_field(field, operator, value)
    case operator
    when "="
      "#{EstimateEntry.table_name}.issue_id = #{value.first.to_i}"
    when "~"
      issue = Issue.where(:id => value.first.to_i).first
      if issue && (issue_ids = issue.self_and_descendants.pluck(:id)).any?
        "#{EstimateEntry.table_name}.issue_id IN (#{issue_ids.join(',')})"
      else
        "1=0"
      end
    when "!*"
      "#{EstimateEntry.table_name}.issue_id IS NULL"
    when "*"
      "#{EstimateEntry.table_name}.issue_id IS NOT NULL"
    end
  end
  ...
  def sql_for_issue_fixed_version_id_field(field, operator, value)
    ...
    when "!"
      if issue_ids.any?
        "#{EstimateEntry.table_name}.issue_id NOT IN (#{issue_ids.join(',')})"
      else
        "1=1"
      end
    end
